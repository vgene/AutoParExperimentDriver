...
def clean_all_bmarks(root_path, bmark_list, reg_option):
    ...
    for bmark in bmark_list:
        ...
        make_process = subprocess.Popen(
            ["make", clean_tgt], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
        )
...
def get_one_prof(root_path, bmark, profile_name, profile_recipe):
    ...
    with open(profile_name.replace(" ", "-") + ".log", "w") as fd:
        make_process = subprocess.Popen(["make", profile_recipe], stdout=fd, stderr=fd)

    if make_process.wait() != 0:
        elapsed = time.time() - start_time
        printAndFlush(
            colored(
                "%s failed for %s, took %.4fs" % (profile_name, bmark, elapsed), "red"
            )
        )
        return False
    else:
        ...
        printAndFlush(
            colored(
                "%s succeeded for %s, took %.4fs" % (profile_name, bmark, elapsed),
                "green",
            )
        )
...
def get_pdg(root_path, bmark, result_path):
    ...
    start_time = time.time()
    make_process = subprocess.Popen(
        ["make", exp_name], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
    )
    ...
    with open("pdg.log", "w") as fd:
        make_process = subprocess.Popen([exp_name, enables], stdout=fd, stderr=fd)

    if make_process.wait() != 0:
        elapsed = time.time() - start_time
        printAndFlush(
            colored(
                "PDG generation failed for %s, took %.4fs" % (bmark, elapsed), "red"
            )
        )
        return False
    else:
        ...
        printAndFlush(
            colored(
                "PDG genration succeeded for %s, took %.4fs" % (bmark, elapsed), "green"
            )
        )
        return True
...
def get_exp_result(
    root_path, bmark, result_path, exp_name="benchmark.collaborative-pipeline.dump"
):
    ...
    if make_process.wait() != 0:
        elapsed = time.time() - start_time
        printAndFlush(
            colored("Experiment failed for %s, took %.4fs" % (bmark, elapsed), "red")
        )
        return None
    else:
        elapsed = time.time() - start_time
        ...
        printAndFlush(
            colored(
                exp_name + " succeeded for %s, took %.4fs" % (bmark, elapsed), "green"
            )
        )
...
def get_seq_time(root_path: str, bmark: str, times: int, use_profile=False):
    ...
    printAndFlush(
        colored("Sequential time measurement succeeded for %s!" % bmark, "green")
    )
...
def get_para_time(root_path, bmark, times, num_workers=28):
    printAndFlush(
        "Try to get parallel execution time, running times is %d, test workers is %d"
        % (times, num_workers)
    )
    os.chdir(os.path.join(root_path, bmark, "src"))
    exp_name = "reg_para"
    ...
    for run_time in range(times):
        make_process = subprocess.Popen(
            ["make", exp_name, "REG_NUM_WORKERS=" + str(num_workers)],
    ...
    printAndFlush(
        colored("Parallel time measurement succeeded for %s!" % bmark, "green")
    )
    printAndFlush(time_list)
...
def get_real_speedup(root_path, bmark, reg_option, times=3, default_num_worker=28):
    ...
    if reg_option == 2:
        ...
        if make_process.wait() != 0:
            ...
            printAndFlush(
                colored(
                    "Real speedup experiment failed for %s, took %.4fs"
                    % (bmark, elapsed),
                    "red",
                )
            )
        ...
        if os.path.exists(exp_name) and os.stat(exp_name).st_size == 0:
            printAndFlush(
                colored(
                    "Hooyah! Same output from sequential and parallel versions for %s! Took %.4fs"
                    % (bmark, elapsed),
                    "green",
                )
            )
            ...
            printAndFlush(
                colored(
                    "Oh snap! Seems like results disagree for %s! Took %.4fs"
                    % (bmark, elapsed),
                    "red",
                )
            )
            if bmark in no_check_list:
                printAndFlush(
                    colored(
                        "%s is in the no checking list! Continue to get results"
                        % (bmark),
                        "green",
                    )
                )
        ...
        para_time_list, para_time = get_para_time(
            root_path, bmark, times, num_workers=default_num_worker
        )
        ...
        if seq_time and para_time and seq_time > 0 and para_time > 0:
            ...
            printAndFlush(
                colored(
                    "Oh snap! Getting execution time failed for %s!" % (bmark), "green"
                )
            )
            return None
    ...
    # Parallel time only
        ...
        for num_workers in num_workers_plan_list:
            para_time_list, para_time = get_para_time(
                root_path, bmark, times, num_workers=num_workers
            )
...
def get_all_passes(
    ...
    status = {}
    if "Inline" in passes:
        status["Inline"] = get_one_prof(
            root_path, bmark, "Inline", "benchmark.inlined.o3.out"
        )
    if "Edge" in passes:
        status["Edge"] = get_one_prof(
            root_path, bmark, "Edge Profile", "benchmark.edgeProf.out"
        )
    if "Loop" in passes:
    ...
    if "Profile-Seq" in passes:
        status["Profile-Seq"] = get_one_prof(
            root_path, bmark, "Profile-Seq", "profile-seq.time"
        )
    if "Asan" in passes:
    ...
    if "Exp-slamp" in passes:
        status["Exp-slamp"] = get_exp_result(
            root_path, bmark, result_path, "slamp.dump"
        )
    ...
    if "Exp-ignorefn" in passes:
        if "Edge" in status and status["Edge"]:
            status["Exp-ignorefn"] = get_exp_result(
                root_path, bmark, result_path, "ignorefn.dump"
            )
        else:
    ...
    if "Exp-3" in passes:
        ...
        if "Edge" in status and status["Edge"] and "LAMP" in status and status["LAMP"]:
            status["Experiment-no-specpriv"] = get_exp_result(
                root_path, bmark, result_path, "no-specpriv.dump"
            )
            status["Experiment-no-specpriv-ignorefn"] = get_exp_result(
                root_path, bmark, result_path, "no-specpriv-ignorefn.dump"
            )
        else:
    ...
    if "No-Spec" in passes:
        status["No-Spec"] = get_exp_result(
            root_path, bmark, result_path, "no-spec.dump"
        )
    if "PDG" in passes:
...
def get_benchmark_list_from_suite(suite, bmark_list):
    ...
    if suite == "All":
        ...
        suite_list = [
            k for k, v in bmark_list.items() if suite in v["suites"] and v["available"]
        ]
    return suite_list
...
def parse_args():
    ...
    parser.add_argument(
        "-p", "--root-path", type=str, help="Root path of CPF benchmark directory"
    )
    parser.add_argument(
        "-b", "--bmark-list", type=str, help="Path to a benchmark json file"
    )
    parser.add_argument(
        "-r",
        "--reg-option",
    ...
    parser.add_argument(
        "-f", "--force-die", action="store_true", help="End the run if any test fails"
    )
    parser.add_argument("-n", "--core-num", type=int, default=4, help="Core number")
    parser.add_argument(
        "-t",
        "--test-times",
        type=int,
    ...
    parser.add_argument(
        "--slamp-workers", type=int, default=1, help="Number of workers for SLAMP"
    )
    ...
    if "LIBERTY_LIBS_DIR" in os.environ and "LIBERTY_SMTX_DIR" in os.environ:
        ...
        print(
            '"LIBERTY_LIBS_DIR" or "LIBERTY_SMTX_DIR" Environment variables are not properly set up'
        )
    ...
    config["result_path"] = os.path.join(
        config["root_path"], "results", dt.strftime("%Y-%m-%d-%H-%M")
    )
...
def preview_config(config):
    ...
    print(
        colored(
            "Please make sure you've committed all changes for both CPF and Regression, otherwise the Git history hash is meanlingless!",
            "red",
        )
    )
    print(colored("##### Configurations #####", "green"))
    print("Benchmark root path: %s" % (colored(str(config["root_path"]), "yellow")))
    print(
        "CPF lib directory: %s, smtx directory: %s, on branch %s, with Git history: %s"
        % (
    ...
    print(
        "Running test on %s benchmarks %s :"
        % (
            colored(str(len(config["bmark_list"])), "yellow"),
            colored(str(config["bmark_list"]), "yellow"),
        )
    )
    ...
    if reg_option == 0:
        ...
        prompt = (
            "Will use old binaries when available, get sequential and parallel results"
        )
    elif reg_option == 3:
    ...
    print("Reg option: %s, %s" % (colored(str(config["reg_option"]), "yellow"), prompt))

    print(
        "Store results under directory: %s" % colored(config["result_path"], "yellow")
    )
...
if __name__ == "__main__":
    ...
    status_list = Parallel(n_jobs=config["core_num"])(
        delayed(get_all_passes)(
            config["root_path"],
            bmark,
            config["passes"],
            config["result_path"],
            config["modules"],
            config["extra_flags"],
            config["slamp_parallel_workers"],
        )
        for bmark in config["bmark_list"]
    )
    if status_list is None:
        print("Experiment failed, status is empty, please check!")
        sys.exit(1)
    ...
    if "RealSpeedup" in config["passes"]:
        ...
        for bmark in config["bmark_list"]:
            real_speedup = get_real_speedup(
                config["root_path"], bmark, config["reg_option"], config["test_times"]
            )
            ...
            if real_speedup:
                if config["reg_option"] == 3:
                    ...
                    print(
                        "For %s, para time: %.2f" % (bmark, real_speedup["para_time"])
                    )
                else:
                    print(
                        "For %s, seq time: %.2f, para time: %.2f, speedup: %.2f"
                        % (
                            bmark,
                            real_speedup["seq_time"],
                            real_speedup["para_time"],
    ...
    reVis = ReportVisualizer(
        bmarks=config["bmark_list"],
